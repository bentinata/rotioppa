...
class Untung_model extends CI_Model{
	...
	function __construct(){
        ...
        $this->tb_rev = 'review';
		$this->tb_harga_baru = 'harga_baru';
		$this->tb_harga_vendor = 'harga_vendor';
		$this->tb_stock = 'stock';
		$this->tb_at_khu = 'atribut_khusus';
	}

	function list_untung($start=false,$limit=false,$justcount=false,$key=false,$filter=false,$filter2=false){
		$k = $this->tb_kat;
		$sk = $this->tb_subkat;
		$sk2 = $this->tb_subkat2;
		$p = $this->tb_produk;
		
		$hb = $this->tb_harga_baru;
		$hv = $this->tb_harga_vendor;
		$stc = $this->tb_stock;
		$atkhu = $this->tb_at_khu;
		...
		if($filter){
			switch($filter){
				...
				#	break;
				case'5':
					$this->db->where("date_format(tgl,'%Y-%m-%d') between '$key[tgl1]' and '$key[tgl2]' and sam_atribut_khusus.stock<='5'");
		...
		$this->db->join($atkhu, "$atkhu.id=$p.id", 'left');
		//$this->db->join($atkhu, "$p.id=$atkhu.id", 'left');
		if($justcount){
		...
		//$this->db->join($sk2, "$p.id=$sk2.id", 'left');
        if($limit!==false)
		$this->db->limit($limit,$start);
		$this->db->select("$p.id,$p.nama_produk,$p.harga_baru, $p.harga_vendor,$atkhu.stock,$atkhu.id,"
            //."$sk.subkategori,$sk.id as idsub,$sk2.subkategori2,"
            //."WHERE stock<=5"
            ,FALSE);
		$this->db->order_by("$p.id,$p.nama_produk");
	...
	function input_produk($id,$idsub,$idsub2,$vcode,$nama,$tgl,$tag,$summary,$desc,$hv,$ha,$hb,$had,$hbd,$ket_diskon='',$aff=false,$kom=0,$berat=1,$mkey='',$mdesc=''){
		$data = array(
			'id'=>$id,
	...
	function update_produk($id,$idsub,$idsub2,$vcode,$nama,$tag,$summary,$desc,$hv,$ha,$hb,$had,$hbd,$ket_diskon='',$aff=false,$kom=0,$berat=1,$mkey='',$mdesc=''){
		$data = array(
			'id_subkategori'=>$idsub,
            'id_subkategori2'=>$idsub2,
            'id_kode_vendor'=>$vcode,
