...
class Testimoni extends Admin_Controller{
	function __construct(){
		...
		/* load model */
		$this->load->model('testimoni_model', 'im');
	}
	function index()
	{
		// paging
		...
		$this->template->set_view ('testimoni',$data);
    }
	function edit($id=false)
	{
		...
		if($this->input->post('_EDIT')){
			...
			$info['testimoni'] = $this->input->post('summary');
            $info['pengirim'] = $this->input->post('title');
		...
		$this->template->set_view ('testimoni_edit',$data);
	}
