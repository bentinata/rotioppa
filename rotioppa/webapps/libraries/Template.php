...
class Template
{
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		if ($this->_layout)
		{
			...
			$template['body'] = $this->parser_special_tag($this->_body); 
	...
	function clear_layout(){
		// hanya untuk menjaga agar tidak error
		$this->set_layout('blankpage');
	}
	function set_params($arr=array())
	{
		foreach($arr as $k=>$v)
		{
			if(isset($this->_partials[$k]))
			{
				$this->inject_partial($k,$v);
			}
			else
			{
				$this->set($k,$v);
	...
	function set_view($view = false, $data = false, $module = false, $vars = 'view') 
	{
		$this->build($view,$data);
	}
	function load_view($page,$data=false)
	{
		echo ci()->load->view($page,$data);
	}
	function load_view_ontheme($page,$data=array())
	{
		echo $this->_load_view($page,$data,FALSE,$this->_theme_path.'/views/');
	}
    function parser_special_tag($content)
	{
		$preg = "/\<specialtag+:(.+?)\>";
		$preg .="(.+?)";
		$preg .="\<\/specialtag\>/s";
		preg_match_all($preg, $content, $matches); #print_r($matches);
		if($matches && count($matches)>0)
		{
			foreach($matches[1] as $key=>$val)
			{
				$vars = explode(':',$val); #print_r($vars);
				if($vars[0]=='metadata')
				{
					$this->_data['template']['metadata'].=''.$matches[2][$key]; 
				}
				elseif($vars[0]=='variable')
				{	
					if(isset($this->_data[$vars[1]]))
						$this->_data[$vars[1]].=$matches[2][$key];
					else
						$this->_data[$vars[1]]=$matches[2][$key];
				}
			}
		...
		return ltrim(preg_replace_callback($preg,function($match){return '';},$content)); // hilangkan tag yg sudah macth
	}
    
    // auto load language based layout name, ada perubahan di fungsi set_layout()
    function load_lang_layout($langname)
    {
        if(file_exists($this->_theme_path.'language/'.ci()->curlang.'/'. $langname.'_lang'. self::_ext($langname)))
        ci()->lang->load($langname,'',FALSE,TRUE,$this->_theme_path); 
    }
}
